...
function ResetGame(){
	...
	for(let row = 1; row <=6;row++)
	{
		...
		{
			let squareElement = document.getElementById("square-" + row + "-" + square);

			if(squareElement != undefined)
			{
				squareElement.classList.remove("wrong-letter");
				squareElement.classList.remove("out-of-place-letter");
				squareElement.classList.remove("correct-letter");
			}
			
			let squareTextElement = document.getElementById("square-text-" + row + "-" + square);

			if(squareTextElement != undefined)
			{
				squareTextElement.classList.remove("invalid-word-text");
	...
	{
		let char = String.fromCharCode(letter).toLowerCase();
		keyElement = document.getElementById("key-" + char);

		if(keyElement != undefined)
		{
			keyElement.classList.remove("correct-letter");
			keyElement.classList.remove("wrong-letter");
			keyElement.classList.remove("out-of-place-letter");
			keyElement.classList.remove("key-colored-text");
		}
	}
	...
	let decrypted = CryptoJS.AES.decrypt(answer, "password");

	let previousAnswer = decrypted.toString(CryptoJS.enc.Utf8);

	do
	{
		answer = possibleWords[Math.floor(Math.random() * possibleWords.length)];
	} while(answer == previousAnswer); //ensures random chance does not allow for the same word twice

	answer = CryptoJS.AES.encrypt(answer, "password");
	...
	currentPossibleGuesses = [];
	currentPossibleAnswers = [];
	for(let i = 0; i <possibleGuesses.length;i++) //reset possible guesses
	{
		currentPossibleGuesses.push(possibleGuesses[i]);
	}
	for(let i = 0;i < possibleWords.length;i++)
	{
		currentPossibleAnswers.push(possibleWords[i]);
	}
	...
	localStorage.setItem('givenUp', 'false');

	let answerBoxElement = document.getElementById("answer-box");
	answerBoxElement.classList.remove("answer-box-display");
}
...
var hardMode = localStorage.getItem("hardMode") == "true";

var currentPossibleGuesses = [];
...
{
	if(keyCode >= 65 && keyCode <= 90 && currentSquare != 6 && !solved && !givenUp)
	{
		let currentSquareText = document.getElementById("square-text-" + currentRow + "-" + currentSquare);

		if(currentSquareText != undefined)
		{
			currentSquareText.innerHTML = String.fromCharCode(('A'.charCodeAt(0) + (keyCode - 65)));
			currentSquare++;
		}

		CheckWordValidity();
	}
	else if(keyCode == 8 && !givenUp)
	{
		let previousSquareText = document.getElementById("square-text-" + currentRow + "-" + (currentSquare - 1));

		if(previousSquareText != undefined)
		{
			previousSquareText.innerHTML = "";
			currentSquare--;
		}

		CheckWordValidity();
	}
	else if(keyCode == 13 && !givenUp)
	{
		let word = "";
		for(let i = 1;i<=5;i++)
		{
			let squareText = document.getElementById("square-text-" + currentRow + "-" + i);

			if(squareText != undefined)
			{
				word+= squareText.innerHTML;
			}
		}

		word = word.toLowerCase();
		if(possibleGuesses.includes(word) && (!hardMode || currentPossibleGuesses.includes(word)))
		{
			SetColors(currentRow);
			localStorage.setItem("row" + currentRow, word);
			currentRow++;
			currentSquare = 1;

			if(autoFill && !solved && !givenUp && currentRow != 7)
			{
				let averages = new Map();
				for(let i = 0; i< currentPossibleAnswers.length;i++)
				{
					averages.set(currentPossibleAnswers[i], AverageWordsLeft(currentPossibleAnswers, currentPossibleAnswers[i]));
				}

				currentPossibleAnswers = AverageSort(currentPossibleAnswers, averages);

				let bestWord = currentPossibleAnswers[0];

				console.log("best words:");
				for(let i = 0;i < 10 && i < currentPossibleAnswers.length;i++)
				{
					let answerString = "";
					if(possibleWords.includes(currentPossibleAnswers[i])) answerString = "(A)";
					console.log((i + 1) + ". " + currentPossibleAnswers[i] + " " + averages.get(currentPossibleAnswers[i]) + " " + answerString);
				}

				if(currentPossibleAnswers.length > 10) console.log("AMOUNT: " + currentPossibleAnswers.length);

				if(bestWord.length == 5)
				{
					bestWord = bestWord.toUpperCase();
					for(let i = 0;i < bestWord.length;i++)
					{
						keyPressed(bestWord.charCodeAt(i));
					}
				}
		...
		{
			console.log("failed hard mode check");	
		}

		if(currentRow == 7 && !solved)
		{
			let answerBoxElement = document.getElementById("answer-box");
			
			answerBoxElement.innerHTML = "The answer was: " + CryptoJS.AES.decrypt(answer, "password").toString(CryptoJS.enc.Utf8);
			answerBoxElement.classList.add("answer-box-display");
...
function AverageSort(answers, averages) {
	for(let i = 0;i < answers.length;i++)
	{
		for(let x = i + 1; x > 0;x--)
		{
			if(averages.get(answers[x - 1]) > averages.get(answers[x]))
			{
				let temp = answers[x - 1];
				answers[x - 1] = answers[x] + "";
				answers[x] = temp + "";
			}
			else if(averages.get(answers[x - 1]) == averages.get(answers[x]) && answers[x] != undefined && answers[x].localeCompare(answers[x - 1]) == -1)
			{
				let temp = answers[x - 1];
				answers[x - 1] = answers[x] + "";
				answers[x] = temp + "";
...
function eliminatePossibleWords(possibleWords, guess, colors, printWatchFor = false)
{
	doesNotInclude = [];
	includes = new Map();
	exactWrongs = new Map();
	exactGuesses = new Map();
	handleSixtyException = [];

	for(let i = 0;i < colors.length;i++)
	{
		if(colors[i] == 'n') doesNotInclude.push(guess.charAt(i));
		else if(colors[i] == 'y')
		{
			if(!includes.has(guess.charAt(i)))
			{
				includes.set(guess.charAt(i), 1);
			}
			else
			{
				includes.set(guess.charAt(i), includes.get(guess.charAt(i)) + 1);
			}

			exactWrongs.set(i, guess.charAt(i));
		}
		else
		{
			exactGuesses.set(i, guess.charAt(i));
		}
	}

	includes.forEach(function(value, key) {
		if(doesNotInclude.includes(key))
		{
			for(let i = doesNotInclude.length - 1; i >=0;i--)
			{
				if(doesNotInclude[i] == key)
				{
					doesNotInclude.splice(i, 1);
				}
			}
		}
	});

	exactGuesses.forEach(function(value){
		if(doesNotInclude.includes(value))
		{
			for(let x = doesNotInclude.length - 1; x >= 0; x--)
			{
				if(doesNotInclude[x] == value)
				{
					for(let i = 0;i < 5;i++)
					{
						if(exactGuesses.has(i)) continue;

						handleSixtyException.push(new Map());
						handleSixtyException[handleSixtyException.length - 1].set(i, value);
					}

					doesNotInclude.splice(x, 1);
				}
			}
		}
		...
		{
			includes.set(value, 1);
		}
		else
		{
			includes.set(value, includes[value] + 1);
		}
	});
	...
	{
		let removed = false;
		for(let x = 0; x < doesNotInclude.length;x++) //remove all words with gray letters
		{
			if(newWords[i].includes(doesNotInclude[x]))
			{
				if(printWatchFor && newWords[i] == watchFor) console.log(newWords[i] + " removed for having a gray letter " + doesNotInclude[x]);
		...
		for(let x = 0; x < newWords[i].length;x++)
		{
			if(letterCounts.has(newWords[i].charAt(x)))
			{
				letterCounts.set(newWords[i].charAt(x), letterCounts.get(newWords[i].charAt(x)) + 1);
			}
			else
			{
				letterCounts.set(newWords[i].charAt(x), 1);
			}
		}

		includes.forEach(function(value, key){
			if(!removed && (!letterCounts.has(key) || letterCounts.get(key) < value)) //remove all words that dont have yellow letters
			{
				if (printWatchFor && newWords[i] == watchFor) console.log(newWords[i] + " removed for not having a yellow letter " + key);
		...
		exactGuesses.forEach(function(value, key) {
			if(!removed && newWords[i].charAt(key) != value)
			{
				if (printWatchFor && newWords[i] == watchFor) console.log(newWords[i] + " removed for not having a green letter on index " + key);
                newWords.splice(i, 1);
                removed = true;
			}
		});
		...
		for(let x = 0; x < handleSixtyException.length && !removed;x++)
		{
			handleSixtyException[x].forEach(function(value, key){
				if(!removed && newWords[i].charAt(key) == value)
				{
					if (printWatchFor && newWords[x] == watchFor) console.log(newWords[i] + " removed for having too many " + value);
		...
		exactWrongs.forEach(function(value, key) {
			if(!removed && newWords[i].charAt(key) == value)
			{
				if (printWatchFor && newWords[i] == watchFor) console.log(newWords[i] + " removed for having a letter in a spot where it was shown yellow and not green on index " + index);
...
function CheckWordValidity()
{
	...
	if(word.length != 5 || (possibleGuesses.includes(word) && (!hardMode || currentPossibleGuesses.includes(word)))) //valid word or not 5 letters
	{
		for(let i = 1;i<=5;i++)
		{
			let squareText = document.getElementById("square-text-" + currentRow + "-" + i);
			...
			if(squareText != undefined)
			{
				squareText.classList.remove("invalid-word-text");
			}
		}
	}
	else
	{
		for(let i = 1;i<=5;i++)
		{
			...
			squareText.classList.add("invalid-word-text");
		}
	}
}
...
function GiveUp()
{
	...
	if(answerBoxElement != undefined && !answerBoxElement.classList.contains("answer-box-display") && !solved)
	{
		answerBoxElement.innerHTML = "The answer was: " + CryptoJS.AES.decrypt(answer, "password").toString(CryptoJS.enc.Utf8);
		answerBoxElement.classList.add("answer-box-display");

		givenUp = true;
		localStorage.setItem('givenUp', 'true');
	}
}
...
function SetColors(row)
{
	...
	let colorText = GetColorString(word);

	currentPossibleGuesses = eliminatePossibleWords(currentPossibleGuesses, word, colorText);
	currentPossibleAnswers = eliminatePossibleWords(currentPossibleAnswers, word, colorText);

	if(colorText == "ggggg") solved = true;

	for(let i = 1; i <= 5;i++)
	{
		let squareElement = document.getElementById("square-" + row + "-" + i);
		...
		if(squareElement != undefined)
		{
			...
			if(colorText.charAt(i - 1) == 'g') squareElement.classList.add("correct-letter");
			else if(colorText.charAt(i - 1) == 'y') squareElement.classList.add("out-of-place-letter");
			else if(colorText.charAt(i - 1) == 'n') squareElement.classList.add("wrong-letter");
		}

		let keyElement = document.getElementById("key-" + word.charAt(i - 1));

		if(keyElement != undefined)
		{
			let alreadyGreen = false;
			let alreadyYellow = false;
			let alreadyWrong = false;
			if(keyElement.classList.contains("correct-letter")) alreadyGreen = true;
			if(keyElement.classList.contains("out-of-place-letter")) alreadyYellow = true;
			if(keyElement.classList.contains("wrong-letter")) alreadyWrong = true;

			if(colorText.charAt(i - 1) == 'g' && !alreadyGreen)
			{
				keyElement.classList.add("correct-letter");
				keyElement.classList.add("key-colored-text");
			}
			else if(colorText.charAt(i - 1) == 'y' && !alreadyYellow && !alreadyGreen)
			{
				keyElement.classList.add("out-of-place-letter");
				keyElement.classList.add("key-colored-text");
			}
			else if(colorText.charAt(i - 1) == 'n' && !alreadyYellow && !alreadyGreen && !alreadyWrong)
			{
				keyElement.classList.add("wrong-letter");
				keyElement.classList.add("key-colored-text");
			}
		}
	}
...
function GetColorString(guess)
{
	let greenCount = new Map();
	let greens = "";

	let actualAnswer = CryptoJS.AES.decrypt(answer, "password").toString(CryptoJS.enc.Utf8);

	for(let i = 0; i< guess.length;i++)
	{
		if(guess.charAt(i) == actualAnswer.charAt(i))
		{
			greens += 'g';

			if(greenCount.has(guess.charAt(i)))
			{
				greenCount.set(guess.charAt(i), greenCount.get(guess.charAt(i)) + 1);
			}
			else
			{
				greenCount.set(guess.charAt(i), 1);
			}
		}
		else
		{
	...
	{
		let char = actualAnswer.charAt(i);
		if(guess.charAt(i) != char)
		{
	...
	for(let i = 0;i < guess.length;i++)
	{
		...
		if(char != actualAnswer.charAt(i) && letterCount.has(char) && letterCount.get(char) >= 1)
		{
			yellows+="y";
			letterCount.set(char, letterCount.get(char) - 1);
		}
	...
	for(let i = 0;i < 5;i++)
	{
		if(greens.charAt(i) == 'g') colors+='g';
		else if(yellows.charAt(i) == 'y') colors+='y';
		else colors+='n';
	}
...
function GenerateSquares(){
	...
	for(let row = 1; row <= 6;row++) //six rows
	{
		...
		for(let square = 1;square <=5;square++)
		{
			...
			squareElement.classList.add("text-dark")
			squareElement.classList.add("center");


			let squareTextElement = document.createElement("div");
			squareTextElement.id = "square-text-" + row + "-" + square;

			squareElement.appendChild(squareTextElement);

			squareTextElement.classList.add("game-square-text");
			squareTextElement.classList.add("game-square-align");
		}
	}
	...
	{
		answer = CryptoJS.AES.encrypt(possibleWords[Math.floor(Math.random() * possibleWords.length)], "password");
		localStorage.setItem("answer", answer);
	...
	else
	{
		...
		for(let i = 1;i <=6;i++)
		{
			let word = localStorage.getItem("row" + i);
		...
		if(localStorage.getItem("givenUp") == "true")
		{
			GiveUp();
		}
	}
	...
	if(hardMode)
	{
		let hardModeSettingsElement = document.getElementById("hard-mode-button");

		if(hardModeSettingsElement != undefined)
		{
			hardModeSettingsElement.classList.add("correct-letter");
		}
	}
	...
	for(let i = 0;i < possibleWords.length;i++)
	{
		...
		let code = CryptoJS.AES.encrypt(possibleWords[i], "password").toString();
...
function AverageWordsLeft(possibleWords, guess)
{
	let colors = "ngy";
		...
		for(let l2 = 0; l2 < 3; l2++)
		{
			for(let l3 = 0; l3 < 3; l3++)
			{
				for(let l4 = 0; l4 < 3; l4++)
				{
					...
					{
						let colorString = colors.charAt(l1) + "" + colors.charAt(l2) + "" + colors.charAt(l3) + "" + colors.charAt(l4) + "" + colors.charAt(l5);

						let words = eliminatePossibleWords(possibleWords, guess, colorString);
	...
	if(possibleCombinations > 0) return wordCount * 1.0 / possibleCombinations;
	return 3000;
}
...
function toggleHardMode()
{
	if(hardMode)
	{
		...
		if(hardModeSettingsElement != undefined)
		{
			hardModeSettingsElement.classList.remove("correct-letter");
		}
		hardMode = false;
		localStorage.setItem('hardMode', 'false');
	}
	else if(currentRow == 1 || solved || givenUp)
	{
		let hardModeSettingsElement = document.getElementById("hard-mode-button");
...
function GenerateKeyBoard()
{
	...
	for(let i = 1;i <=3;i++)
	{
		...
		if(i == 3)
		{
			...
			newKey.classList.add("center");
			newKey.classList.add("side-key");

			newKey.innerHTML = "ENTER";

			newKey.onclick = function () { keyPressed(13) };
		}
		for(let x = 0; x < rows[i - 1].length;x++)
		{
			let newKey = document.createElement("div");
			newKey.id = "key-" + (rows[i - 1].charAt(x) + "").toLowerCase();
			...
			newKey.innerHTML = rows[i - 1].charAt(x) + "";

			newKey.onclick = function () { keyPressed(rows[i - 1].charCodeAt(x)) };
		}
		...
		if(i == 3)
		{
			...
			newKey.innerHTML = "BACK";

			newKey.onclick = function () { keyPressed(8) };
		}
	}
}
